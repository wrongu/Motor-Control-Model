...
servoNames = {'TorsoYaw', 'TorsoPitchOne', 'TorsoPitchTwo', 'RightShoulderRotator', 'RightShoulderPitch',...  
    'RightElbow', 'RightWrist','LeftShoulderRotator','LeftShoulderPitch','LeftElbow','LeftWrist'};
timings = struct;
for k=1:length(servoNames)
    timings.servo{k} = struct;
    %first column is speed setting, columns 2-10 are trial speeds
    timings.servo{k}.trials = zeros(10,numTrials); 
    %first column is speed setting, second column is mean speed across
    %trials
    timings.servo{k}.speed = zeros(10,2);
end
