...
private:
	/// root is assumed to be at end of the vector
	vector< pair<pair<Tkey,Tkey>,int> > heap;
	/**
	 *  maintain a list of back indexes.
	 *  * -1           not in heap
	 *  * other 	   index that point to cell in vector heap
...
public:
	...
	/// pushes a new value in the heap
	void push( Tkey key1, Tkey key2, int index ){
		//cout << "pushing " << index << endl;
		if( useBackIdx && index >= (int) backIdx.size() )
			throw InvalidIndexException("the index in the push must be smaller than the maximal allowed index (specified in constructor)");
		...
		if( !useBackIdx ){
			// add to the back of the vector
            pair<Tkey,Tkey> keyPair = make_pair(key1,key2);
			heap.push_back( make_pair(keyPair, index) );
			// recursive call to increase key
			heapDecreaseKey( heap.size()-1, keyPair.first, keyPair.second );
		}
		else{
			if( backIdx[index] == -1 ){
				// add to the back of the vector
				...
				//initially point to back
				backIdx[ index ] = heap.size()-1;
				// recursive call to decrease key
				heapDecreaseKey( heap.size()-1, keyPair.first, keyPair.second );
				// USE STL STUFF
				//push_heap(heap.begin(),heap.end());
			}
			// update push (a key exists)
                ...
                pair<Tkey,Tkey> keyPair = make_pair(key1,key2);
				heapDecreaseKey( backIdx[index], keyPair.first, keyPair.second );
			}
		}
	}
	
	/// return a constant reference to the MINIMAL KEY element stored in the head of the heap
	const pair<pair<Tkey,Tkey>,int>& top() throw(HeapEmptyException){
		if( heap.empty() )
			throw new HeapEmptyException("Impossible to get top element, empty heap");
		else
			return heap[0];
    ...
    //Modified by Divya Gunasekaran (May 6, 2011) to update backIdx vector as well
	/// removes the top element of the queue (minimal)
	void pop() throw(HeapEmptyException){
		if( heap.size() < 1 ) //a.k.a. heap.empty()
			throw new HeapEmptyException("heap underflow");
		...
		// overwrite top with tail element
        heap[0] = heap.back();	
        //Update backIdx vector
        if( useBackIdx ){
			//backIdx[ (heap.back()).second ] = 0;
		}
    ...
    //Added by Divya Gunasekaran (May 6, 2011)
    //removes the element with the given index from the priority queue
    //can only be used if backindexes are used
    void remove(int index){
        //If backindexes are used and the given index is in the heap
        if(useBackIdx && backIdx[index] != -1){
            
            int lastPos = backIdx[index]; //original position
            
            //Swap element to be removed with last element in the heap
            swap(lastPos, heap.size()-1);
            
            //Clear the backIdx associated with the element we are removing
            backIdx[index] = -1;
            
            //Delete the last element (which is the one we want to remove now)
            heap.pop_back();
            
            //Start heapify from original location
            if(heap.size() > 1){
                minHeapify(lastPos);
            }
        }     
...
private:
	/// check and applies MinHeap Correctness down the subtree with index "currIdx"
	void minHeapify(int currIdx){
		unsigned int leftIdx = LEFT( currIdx );
		unsigned int rightIdx = RIGHT( currIdx );
		
		// decide if and where ta swap, left or right, then swap
		// current is the best choice (default)
		int smallerIdx;
		
		// is left a better choice? (exists an invalid placed smaller value on the left side)
		if( leftIdx < heap.size() && (heap[leftIdx].first.first < heap[currIdx].first.first || (heap[leftIdx].first.first == heap[currIdx].first.first && heap[leftIdx].first.second < heap[currIdx].first.second)))
			smallerIdx = leftIdx;
		else
			smallerIdx = currIdx;
		
		// is right a better choice? (exists an invalid placed smaller value on the right side)
		if( rightIdx < heap.size() && (heap[rightIdx].first.first < heap[smallerIdx].first.first || (heap[rightIdx].first.first == heap[smallerIdx].first.first && heap[rightIdx].first.second < heap[smallerIdx].first.second)))
			smallerIdx = rightIdx;
	...
	void swap(int pos1, int pos2){
		...
		// update heap
		pair<pair<Tkey,Tkey>,int> temp = heap[pos1];
		heap[pos1] = heap[pos2];
		heap[pos2] = temp;
	}
	
	/// propagates the correctness (in heap sense) down from a vertex currIdx
	void heapDecreaseKey( int currIdx, Tkey key1, Tkey key2 ){
		// check if given key update is actually an increase
		if( key1 > heap[currIdx].first.first || (key1 == heap[currIdx].first.first && key2 > heap[currIdx].first.second) )
			throw InvalidKeyIncreaseException("In MinHeaps only decrease in key updates are legal");

		// update value with current key
		heap[currIdx].first.first = key1;
        heap[currIdx].first.second = key2;
				
		// traverse the tree up making necessary swaps
		int parentIdx = PARENT(currIdx);
		while( currIdx > 0 ){
			if( heap[ parentIdx ].first.first > heap[ currIdx ].first.first || (heap[ parentIdx ].first.first == heap[ currIdx ].first.first && heap[ parentIdx ].first.second > heap[ currIdx ].first.second) ){
				// make swap
				swap( currIdx, parentIdx );
				// move up
				currIdx = parentIdx;
