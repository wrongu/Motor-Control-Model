...
function [path,Movement,tElapsed,B] = A_ComputePath2(currentposture,goalPt,B,thresh)
...
%        adjCells = A_Succ_reduced(B,cellIndex, n);
...
%     disp('Adjacent cells:');
    ...
    %adjCells = A_Succ_reduced(B,curr,n);
    
    adjCells = adjCells(~[B(adjCells).obstacle])
     currentposture
    
    
%     for i=1:numel(adjCells)
%         adjIndex = adjCells(i);
%         %cost = euclidDist(B(curr).center,B(adjIndex).center)...
%                 %+ euclidDist(B(adjIndex).center,goalPt);
%         cost =  euclidDist(B(curr).center,B(adjIndex).center) ...
%                 + B(adjIndex).g;
%         if(cost <= minCost)
%             minCost = cost;
